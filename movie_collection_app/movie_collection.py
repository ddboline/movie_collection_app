...
class MovieCollection(object):
    ...
    def read_queue_from_db(self):
        ...
        self.current_queue = []
        for idy, row in enumerate(self.con.execute('select * from current_queue order by idx')):
    ...
    def write_queue_to_db(self):
        for idy, row in enumerate(self.current_queue):
            ...
            self.con.execute("insert into current_queue (index, idx, path, show) values "
            ...
            for itdict in epi_rating_items:
                ...
                if tmp and tmp.get((season_, episode_), {}):
                    self.con.execute("UPDATE imdb_episodes SET "
                                     ...
                                     "and episode = %d" % (rating_, epurl_, airdate_, eptitle_,
                                                           show, season_, episode_))
                else:
    ...
    def get_season_episode_rating_from_name(self, fname):
        ...
        show, link = map(ratings.get, ('show', 'link'))
        ...
        if show not in self.imdb_episode_ratings:
            ...
            self.get_imdb_episode_ratings(show, season=season)
        elif show not in dailies and (self.imdb_episode_ratings.get(show).get(
            (season, episode)).get('rating') < 0):
            self.get_imdb_episode_ratings(show, season=season)
    ...
    def add_entry(self, fname, position=-1):
        ...
        if fname in self.queue_dict:
            ...
            self.con.execute("update current_queue set idx = idx + 1 "
                             "where idx >= %d and path != '%s'" % (position, fname))
            if fname not in self.movie_collection:
                idx_ = list(self.con.execute("select max(idx) from movie_collection"))[0][0]
                if idx_ is None:
                    idx_ = 0
                else:
                    idx_ += 1
                self.con.execute("insert into movie_collection (idx, path, show) values "
    ...
    def rm_entry(self, position, purge=False):
        ...
        try:
            ...
            self.con.execute("delete from current_queue where idx = %d" % tmp_val['idx'])
            self.con.execute("update current_queue set idx = idx - 1 "
    ...
    def rm_entry_from_collection(self, fname):
        if fname not in self.movie_collection:
            return
        self.con.execute("delete from movie_collection where path='%s'" % fname)
    ...
    def entry_string(self, idx, do_time=False):
        ...
        if not do_time:
            ...
            tm_ = read_time(fname)
            if tm_ > 0:
                out_list += ['%4d %s %s %s' % (idx, print_h_m_s(tm_), fname, link)]
        return out_list
    ...
    def list_entries(self, name=None, first_entry=0, last_entry=0, do_time=False, do_local=False):
    ...
    def list_entries_from_collection(self, name=None, do_time=False, do_local=False):
        out_list = []
    ...
    def list_tvshows(self, do_random=False, play_random=False):
        ...
        if do_random:
            ...
            for show in list_of_tvshows:
                out_list += ['%-30s %3d %10s' % (show, tvshows[show], imdb_link[show])]
        return out_list
